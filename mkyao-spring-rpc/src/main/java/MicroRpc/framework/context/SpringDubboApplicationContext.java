...
public class SpringDubboApplicationContext extends AbstractApplicationContext{
    ...
    public <T> T getService(Class<T> clazz,int waitSec,DemotePolicy demotePolicy,Class fallbk,int retryCnt){
        return this.getProtocol().getService(clazz,waitSec,demotePolicy,fallbk,retryCnt);
    }
        ...
        for (String beanDefinitionName : beanDefinitionNames) {
            ...
            if (fieldList!=null && !fieldList.isEmpty()){
                for (Field field : fieldList) {
                    ...
                    try {
                        ServiceRefrence annotation = field.getAnnotation(ServiceRefrence.class);
                        int waitSec = annotation.waitSec();
                        DemotePolicy demotePolicy = annotation.demotePolicy();
                        Class fallback = annotation.fallback();
                        int retrycnt = annotation.retrycnt();
                        refrencesWaittimeMp.put(field.getType(),waitSec);
                        field.set(bean,this.getService(field.getType(),waitSec,demotePolicy,fallback,retrycnt));
